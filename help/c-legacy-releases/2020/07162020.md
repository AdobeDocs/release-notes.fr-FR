...
Consultez la [documentation du produit](https://experienceleague.adobe.com/docs/core-services/interface/experience-cloud.html?lang=fr) de l’interface d’Experience Cloud pour obtenir un exemple.
...
* [Avis importants à lʼintention des administrateurs dʼAnalytics](#aa-notices) (mise à jour : mardi 13 juillet 2020)
...
| API Data Repair – version Beta publique | 14 juillet 2020 | L’[!UICONTROL API Data Repair] vous offre un mécanisme de suppression ou de modification de certaines données Adobe Analytics existantes. Les demandes de [!UICONTROL réparation de données] sont effectuées en envoyant une définition de travail à l’[!UICONTROL API Data Repair], qui inclut la suite de rapports, la période, les variables et les actions à appliquer aux données. Lors du lancement de la version Beta publique, l’[!UICONTROL API Data Repair] prend en charge la suppression des données d’[!UICONTROL Activity Map]. D’autres fonctionnalités seront déployées ultérieurement. Contactez l’Assistance clientèle pour rejoindre la version Beta publique de l’API Data Repair. [En savoir plus…](https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/data-repair.md) |
...
* **Dynamic Media Classic**
  ...
  Voir [Application de bureau Dynamic Media Classic d’Adobe – Maintenant disponible.](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/new-ui-2020.html?lang=fr)
...
* **Ajout de la prise en charge des ressources 3D à Dynamic Media**
   ...
   * Dans AEM as a Cloud Service, consultez [Utilisation de ressources 3D dans Dynamic Media.](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/dynamicmedia/assets-3d.html?lang=fr)
   * Dans AEM 6.5, consultez [Utilisation de ressources 3D dans Dynamic Media.](https://experienceleague.adobe.com/docs/experience-manager-65/assets/dynamic/assets-3d.html?lang=fr)
...
* **Mises à jour de la documentation d’AEM 6.5.5 Forms**
   ...
   * Nouvelles fonctionnalités et améliorations de la version 6.5.5 :
      ...
      * [Personnalisez les colonnes de la boîte de réception d’Adobe Experience Manager](https://experienceleague.adobe.com/docs/experience-manager-65/authoring/essentials/inbox.html?lang=fr#inbox-admin-control).
      * [Enregistrez les communications interactives en tant que brouillon.](https://experienceleague.adobe.com/docs/experience-manager-65/forms/interactive-communications/prepare-send-interactive-communication.html?lang=fr#save-as-draft)
      * Prise en charge du serveur d’applications Oracle WebLogic pour les installations de [serveur unique](https://helpx.adobe.com/content/dam/help/fr/experience-manager/6-5/forms/pdf/prepare-install-single-server.pdf) et de [cluster](https://helpx.adobe.com/content/dam/help/fr/experience-manager/6-5/forms/pdf/prepare-install-cluster.pdf).
      * [Amélioration de l’accessibilité.](https://experienceleague.adobe.com/docs/experience-manager-65/release-notes/service-pack/new-features-latest-service-pack.html?lang=fr#accessibility-improvements)
      * [Authentification basée sur certificat X-509 pour les services web basés sur SOAP dans le modèle de données de formulaire.](https://experienceleague.adobe.com/docs/experience-manager-65/forms/form-data-model/configure-data-sources.html?lang=fr#configure-soap-web-services)
      * [Prise en charge d’Oracle RAC.](https://experienceleague.adobe.com/docs/experience-manager-65/release-notes/service-pack/new-features-latest-service-pack.html?lang=fr#other-improvements)
      * [Amélioration de la journalisation des erreurs dans les rapports de transaction.](https://experienceleague.adobe.com/docs/experience-manager-65/forms/transaction-reports/viewing-and-understanding-transaction-reports.html?lang=fr#view-transaction-reporting-logs)
   * Nouvelles fonctionnalités et améliorations de la version 6.4.8.1 :
      * [Authentification basée sur certificat X-509 pour les services web basés sur SOAP dans le modèle de données de formulaire.](https://experienceleague.adobe.com/docs/experience-manager-64/forms/form-data-model/configure-data-sources.html?lang=fr#configure-soap-web-services)
      * [Amélioration de la journalisation des erreurs dans les rapports de transaction.](https://experienceleague.adobe.com/docs/experience-manager-64/forms/transaction-reports/viewing-and-understanding-transaction-reports.html?lang=fr#view-transaction-reporting-logs)
...
| -----------| ---------- | ---------- | ---------- |
| 25 juin 2020 | [Prise en main des formulaires adaptatifs](https://experienceleague.adobe.com/docs/experience-manager-learn/forms/creating-your-first-adaptive-form/adaptive-forms-getting-started-tutorial-use.html?lang=fr) | Vidéo | Ces tutoriels décrivent les étapes de création d’un formulaire adaptatif à plusieurs onglets. Découvrez comment utiliser des tableaux, une disposition en accordéon et un éditeur de règles pour créer des règles métier. |
| 25 juin 2020 | [Création d’un processus d’examen dans AEM Forms](https://video.tv.adobe.com/v/3410034?captions=fre_fr) | Vidéo | Découvrez comment créer un processus pour examiner les données envoyées à partir d’un formulaire actif. |
| 23 juin 2020 | [Profils de traitement](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/configuring/processing-profiles.html?lang=fr) | Vidéo | Les profils de traitement définissent les rendus à créer pour les ressources dans AEM as a Cloud Service. |
| 23 juin 2020 | [Bonnes pratiques de Dynamic Media Classic](https://experienceleague.adobe.com/docs/experience-manager-learn/dynamic-media-classic-tutorial/overview.html?lang=fr) | Article | Les utilisateurs actuels et les nouveaux utilisateurs peuvent en savoir plus sur Dynamic Media Classic, ses principales capacités et le processus de _création_, d’_auteur_ et de _diffusion_. |
| 23 juin 2020 | [Débogage de la version et des déploiements d’AEM as a Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/build-and-deployment.html?lang=fr) | Article | Découvrez comment déboguer la version et les déploiements d’AEM as a Cloud Service. |
| 16 juin 2020 | [Débogage d’AEM as a Cloud Service à l’aide de journaux](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/logs.html?lang=fr) | Article | Découvrez comment utiliser les journaux pour déboguer AEM as a Cloud Service. Les journaux se trouvent en première ligne du débogage des applications AEM, mais dépendent d’une journalisation adéquate dans l’application AEM déployée. |
| 5 juin 2020 | [Projet de l’éditeur d’application d’une seule page](https://experienceleague.adobe.com/docs/experience-manager-learn/spa-react-tutorial/create-project.html?lang=fr) | Article | Découvrez comment utiliser l’archétype du projet Adobe Experience Manager (AEM) pour générer un projet Maven à plusieurs modules comme point de départ pour une application React intégrée à l’éditeur d’application d’une seule page AEM. |
...
| 8 juillet 2020 | [Installation et configuration du client Adobe Campaign](https://experienceleague.adobe.com/docs/campaign-classic-learn/tutorials/getting-started/install-and-setup-the-adobe-campaign-client.html?lang=fr) | Campaign Classic | Apprenez à télécharger et installer la console du client Adobe Campaign, à créer et gérer vos connexions à plusieurs environnements et à vérifier l’accès à la console du client Adobe Campaign. |
| 19 juin 2020 | [Présentation d’Adobe Campaign Classic](https://experienceleague.adobe.com/docs/campaign-classic-learn/tutorials/getting-started/introduction-to-adobe-campaign-classic.html?lang=fr) | Campaign Classic | Découvrez la place d’Adobe Campaign Classic dans le portefeuille d’expériences digitales d’Adobe, ainsi que les principales fonctionnalités et capacités. |
| 12 juin 2020 | [Déploiement d’un modèle de diffusion de courrier électronique ad hoc](https://experienceleague.adobe.com/docs/campaign-classic-learn/tutorials/sending-messages/using-delivery-templates/deploying-ad-hoc-email-delivery-template.html?lang=fr) | Campaign Classic | Découvrez comment déployer un modèle de courrier électronique ad hoc. |
| 12 juin 2020 | [Configuration d’un modèle de diffusion](https://experienceleague.adobe.com/docs/campaign-classic-learn/tutorials/sending-messages/using-delivery-templates/configuring-a-delivery-template.html?lang=fr) | Campaign Classic | Découvrez comment configurer un modèle de courrier électronique. |
| 12 juin 2020 | [Définition des propriétés du modèle de diffusion](https://experienceleague.adobe.com/docs/campaign-classic-learn/tutorials/sending-messages/using-delivery-templates/setting-delivery-template-properties.html?lang=fr) | Campaign Classic | Découvrez comment définir les propriétés du modèle de courrier électronique. |
| 26 juin 2020 | [Prise en main de l’interface utilisateur dans Adobe Campaign Standard](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/getting-started/getting-started-with-the-ui.html?lang=fr) | Campaign Standard | Cette vidéo présente l’interface utilisateur d’Adobe Campaign Standard et explique comment accéder aux fonctionnalités clés et de base. |
| 26 juin 2020 | [Gestion des clés GPG](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/control-panel/gpg-key-management/gpg-key-management-overview.html?lang=fr) | Campaign Standard/Panneau de contrôle | Découvrez comment générer et installer une paire de clés GPG publique/privée pour le chiffrement des données et comment importer et installer une clé publique pour le déchiffrement des données. |
...
| Fonctionnalité | Description |
| -----------| ---------- |
| Le GDPR Transparency and Consent Framework (Cadre de transparence et de consentement RGPD) | Advertising Cloud met à niveau sa prise en charge du Transparency and Consent Framework de l’IAB vers la version 2.0 d’ici le 15 août 2020. Pour l’instant, aucune modification n’est nécessaire côté client.  En tant que responsable du traitement des données pour chacun de nos clients, Adobe Advertising Cloud dispose des capacités nécessaires pour permettre aux clients et aux clientes de remplir leurs obligations en vertu du RGPD qui s’appliquent à l’utilisation de nos produits et services. |
| Intégration d’Audience Manager | (Version Beta pour les clientes et clients Adobe Audience Manager avec Adobe Experience Platform uniquement) DSP peut envoyer des fichiers journaux exploitables qui contiennent les données d’engagement publicitaire d’un annonceur dans Audience Manager par le biais d’Adobe Experience Platform. Dans Audience Manager, les publicitaires peuvent utiliser les données pour une segmentation avancée et une utilisation dans les outils d’insights, tels que les rapports d’Audience Optimization. |
| Ciblage graphique inter-périphériques basé sur les personnes | Les clients et clientes en libre-service DSP peuvent utiliser une représentation graphique des périphériques (Adobe Device Co-op ou LiveRamp) pour le ciblage basé sur les personnes et la gestion des fréquences dans les nouvelles campagnes. Cela permet aux publicitaires d’atteindre leurs audiences via leurs propres appareils tout en limitant leur exposition. En outre, la modification en masse des paramètres de ciblage inter-périphériques à travers les emplacements est désormais en version Beta. Pour activer la version Beta sur votre compte, contactez votre gestionnaire de compte Adobe. |
| Ciblage des sites | Les expressions « listes bloquées » et « listes autorisées » ont été renommées en termes plus juste : « (sites) bloqués » et « (sites) ciblés ». |
| Annonces Flashtalking | DSP insère désormais automatiquement les macros de clics pour toutes les balises d’annonces Flashtalking au moyen des workflows de création d’annonces uniques et tierces en bloc. |
| Rapports | (Version Beta uniquement pour les clientes et clients Adobe Experience Platform) Un nouveau rapport intercanal dans Advertising Cloud Search fournit une vue unifiée des dépenses média sur DSP et Search, avec des mesures clés quotidiennes pour les emplacements DSP et les campagnes Search. |
...
| Fonctionnalité | Description |
| -----------| ---------- |
| [!UICONTROL Alertes Beta] | Vous pouvez maintenant ouvrir une vue en lecture seule et filtrée contenant les données de n’importe quelle alerte, puis ouvrir une vue filtrée des entités dans la vue de gestion de campagne concernée, à partir de laquelle vous pouvez modifier les enregistrements d’entité. |
