...
| [!UICONTROL Campagnes], [!DNL Advanced Campaign Management], [!UICONTROL Rapports] | (Mise à jour du 16 juillet) (Campagnes [!DNL Google Ads] et [!DNL Microsoft Advertising]) La prise en charge suivante est désormais disponible pour les annonces de recherches réactives :<ul><li>La vue [!UICONTROL Annonces] affiche désormais les prévisualisations des annonces de recherches réactives.</li><li> (Depuis le 20 juin) Vous pouvez désormais créer des variations d’annonces de recherches dynamiques réactives à l’aide d’un modèle d’annonce spécifique au moteur de recherche, en fonction du contenu de votre inventaire, à partir de [!UICONTROL Campagnes] > [!UICONTROL Avancé (ACM)].</li><li>Le [!UICONTROL Rapport sur les variations d’annonces] comprend deux nouvelles colonnes personnalisées : « [!UICONTROL Titres créatifs] », qui est une liste séparée par des virgules des lignes de titre de l’annonce, et « [!UICONTROL Descriptions] », qui est une liste séparée par des virgules des lignes de description de l’annonce.</li></ul> |
