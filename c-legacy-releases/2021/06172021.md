...
|  | Un nouveau type d’alerte est disponible, [!UICONTROL Actions de l’interface utilisateur de Campaign Management], qui affiche le statut final de toutes les tâches en arrière-plan, y compris les modifications en masse et les modifications d’affectation apportées aux portfolios, aux contraintes, aux classifications et aux événements. L’abonnement aux alertes relatives aux actions de l’interface utilisateur est automatique.<br><br>Ce type d’alerte remplacera à terme les notifications de statut des tâches héritées accessibles dans la barre d’outils de droite (notifications de statut des tâches héritées). |
